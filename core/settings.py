...
db_from_env = dj_database_url.config(conn_max_age=600, ssl_require=True if os.getenv('DATABASE_URL') else False)

DATABASES = {
    'default': db_from_env if os.getenv('DATABASE_URL') else {
...
TORRENT_FILES_ROOT = os.path.join(MEDIA_ROOT, "torrents")
os.makedirs(TORRENT_FILES_ROOT, exist_ok=True)
...
if os.getenv("B2_BUCKET_NAME"):
    ...
    AWS_S3_CUSTOM_DOMAIN = os.getenv("B2_PUBLIC_DOMAIN")
    MEDIA_URL = f"https://{os.getenv('B2_PUBLIC_DOMAIN')}/"